...
function renderSubtasks() {
  ...
  const renderTree = (tasks, depth) => {
    for (const st of tasks) {
      ...
      const retrying = st.retry_count > 0 && !exhausted;
      const stuck = st.retry_count >= 2 && !exhausted;
      const retryBadge = exhausted ? '<span class="retry-badge" title="重试耗尽，已被重新分解">🔄</span>'
        : stuck ? `<span class="stuck-badge" title="已重试${st.retry_count}次，可能需要人工介入">🔴 需介入</span>`
        : retrying ? `<span class="retrying-badge" title="第${st.retry_count}次重试">⚠️ 重试${st.retry_count}</span>`
        : '';
      ...
      html += `<div class="st${active}${leader}" data-id="${st.id}" style="padding-left:${indent + 8}px">
        ...
        <div class="st-meta">
          ...
          ${st.state === 'done' ? '<span class="done-badge">✅ 已完成</span>' : ''}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $('#feedback-input').onkeydown = (e) => {
    if (e.key === 'Enter') $('#feedback-send').onclick();
  };
