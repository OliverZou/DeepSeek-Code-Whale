...
Whale 现在拥有一套多层次的代码修改自律机制，确保 Agent 的每一次改动都精准、可验证、可追溯：

| 层级 | 机制 | 说明 |
|------|------|------|
| **P1** | 先读后改门禁 | 当前回合未读取过的文件，直接拒绝 edit/write/multi_edit 操作，杜绝盲改 |
| **P2** | 改后自动验证 | 每次代码修改后自动运行 build + lint 检查，失败则触发修复闭环 |
| **P3** | 最小变更原则 | immutable prompt 强制约束：不改无关代码、不加未要求的功能、优先用 edit 而非 overwrite |
| **P4** | 根因分析 | 修改前须输出机器可读的 `<analysis>` 结构（root_cause / fix_strategy / files_to_change），Agent 交叉校验实际 diff |

### 🔄 验证-修复闭环（Verify-Fix Loop）

修改完成后自动进入验证-修复循环：运行构建/静态检查 → 发现问题 → 将结果注入回主循环让模型自动修复 → 再次验证，最多多轮。支持 flaky 检测避免无限循环，子 agent 产生的变更也会被追踪。

### 📋 任务驱动终止

Agent 在退出前检查未完成的 todo 并提醒用户；强制总结时输出结构化格式（已完成 / 待处理 / 下一步）。

### 🔍 agent_search 工具

新增 `agent_search` 工具，让 Agent 能够按需发现并委托给专业自定义子 agent（安全审计、数据库、交易、部署等），无需手动配置。

### ⚡ 其他增强

- **Deferred MCP** 工具注入系统 prompt，解除搜索上限
- **子 agent 报告溢出处理**：超长报告自动写入文件而非截断
- **并行验证**：auto-verify、auto-test、review agent 三路并行执行
- **第三方 Review Agent**：可选配第三方模型对改动进行独立审查
