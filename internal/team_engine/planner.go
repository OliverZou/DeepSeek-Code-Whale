...
某个模块的核心实现（纯函数/纯逻辑）和它的单元测试，是同一个关注点，
必须合并为同一个任务、由同一个 worker 同时产出（在同一目录写实现文件和测试文件）。
原因：
- 测试要 require/import 同目录的实现文件，同 worker 产出才能保证加载路径正确；
- 同一个 worker 写实现和测试，两者语义一致，自动化测试门才能稳定通过；
- 拆成两个任务会引入跨 worker 的文件依赖和语义错位，导致机械门反复失败重试。

若确因体量过大必须拆开，测试任务必须 depends_on 实现任务（串行执行）。
只有跨模块的集成测试（涉及 DOM/网络/多模块协作）才拆成独立任务。

## depends_on_batch 使用规则（关键）

depends_on_batch 声明「下游任务依赖其产出的上游 batch」——无论是读取还是修改。
以下情况必须声明 depends_on_batch：
- 下游任务要读取上游产出的文件（如 ui.js 读 game.js 的函数、读 index.html 的 DOM 契约）；
- 下游任务会修改/覆盖上游任务产出的文件。
...
- 结构 ↔ 样式（HTML 的 class/id ↔ CSS 选择器，两边各发明一套命名 = 全部样式失效）；
- 接口 ↔ 实现（函数/模块签名 ↔ 调用方）；
- 数据 ↔ 消费（数据格式 ↔ 解析方）。

分解时必须显式指定契约，禁止让并行任务各自发明接口：
1. 把「契约定义权」指派给产出结构/接口/数据的那一方（如 index.html 定义 DOM）；
2. 在定义方任务的 description 中写出契约的具体内容（类名、id、字段名、函数签名）；
3. 在所有耦合任务的 description 中附上同一份契约要点，保证每个 worker 拿到的
   description 都自含契约（例：样式任务的 description 写明「DOM 契约由 index.html
   定义：.cell/.score-box/.score-value/.overlay/.btn，CSS 必须使用这些选择器」）。
...
目标拆成多个任务后，产出物之间常有真实文件引用（网页 <script>/<link> 引用 js/css、
文档章节互链、模块 import 调用、报告引用调研）。这类引用只要断一处，整体交付物就
无法工作（例：页面漏引 ui.js，只剩空棋盘、游戏不可玩）。

分解完成后必须自查并满足：
1. 引用双向闭环——引用方任务（如 index.html）的 description 与 acceptance_criteria
   显式列出它引用的全部上游产出（「引入 game.js、style.css、ui.js」并列为验收项）；
   被引用方任务（如 ui.js）的 description 注明「本产出被 index.html 引用」。
   写下游任务时，回头补全上游任务的引用清单，保证两边一致。
2. 追加集成验证任务——当目标最终交付为一个可运行/可阅读的整体（网页、应用、文档集、
   研究报告、设计方案等），额外产出一个 title 含「集成验证/端到端验收」的任务，
   depends_on 所有上游 batch，role 用对应类型的验证角色（代码类=软件 QA 工程师），
