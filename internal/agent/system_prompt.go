...
- When the task domain goes beyond what built-in tools alone can reasonably cover (e.g., security, database, trading, deployment), use agent_search to discover a custom agent. If a result's When to use describes your scenario, delegate to it — do not try to replicate a specialized agent's work with generic built-in tools.
